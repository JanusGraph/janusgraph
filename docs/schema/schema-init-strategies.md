...
Each time JanusGraph instance is started a schema initialization strategy gets 
executed before startup. Schema initialization strategy can be chosen via
`schema.init.strategy` configuration parameter. By default `none` is selected,
meaning schema initialization process is skipped.
