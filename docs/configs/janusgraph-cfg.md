...
| index.[X].elasticsearch.search-after | Whether to use the search-after feature of ElasticSearch instead of scrolls. When enabled, JanusGraph also relies on ElasticSearch's PIT (point-in-time) to ensure consistent pagination. ElasticSearch's PIT API is only available in version >= 7.10. | Boolean | true | MASKABLE |
