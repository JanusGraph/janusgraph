...
public abstract class JanusGraphSpeedTest extends GroovySpeedTestSupport {
     ...
     * c.t.janus.testutil.gen.{Schema, GraphGenerator}.
     */
...
//                JanusKey uidKey = tx.getPropertyKey(Schema.UID_PROP);
...
//    }
    ...
    private void multiVertexQueryTask(JanusTransaction tx, JanusVertex[] vbuf, int vcount) {
        if (vcount != vbuf.length) {
            def newbuf = new JanusVertex[vcount]
        ...
        for (int i = 0; i < schema.edgeLabels; i++) {
            Map<JanusVertex, Iterable<JanusEdge>> m = tx.multiQuery(vbuf).labels(schema.edgeLabelNames[i]).edges()
            for (Iterable<JanusEdge> iter : m.values()) {
                for (JanusEdge e : iter) {
                    n++
                }
            }
        }
    ...
    private void singleVertexQueryTask(JanusTransaction tx, JanusVertex v) {
        int n = 0
        for (int i = 0; i < schema.edgeLabels; i++) {
            for (JanusEdge iter : v.query().labels(schema.edgeLabelNames[i]).edges()) {
                n++
            }
        }
        assertTrue(0 < n)
