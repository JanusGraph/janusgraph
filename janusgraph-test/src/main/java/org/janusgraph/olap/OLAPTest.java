...
public abstract class OLAPTest extends JanusGraphBaseTest {
    ...
    private int generateRandomGraph(int numV) {
        ...
        for (int i=0;i<numV;i++) {
            ...
            for (int j=0;j<edges;j++) {
                JanusVertex u = vs[random.nextInt(numV)];
                v.addEdge("knows", u);
                numE++;
            }
        }
    ...
    public void removeGhostVertices() throws Exception {
        ...
        JanusVertex v2 = tx.addVertex("person");
        v1.property("name","marko");
        JanusVertex v3 = tx.addVertex("person");
        ...
        JanusTransaction xx = graph.buildTransaction().checkExternalVertexExistence(false).start();
        v3 = getV(xx, v3id);
        assertNotNull(v3);
        v1 = getV(xx, v1id);
        assertNotNull(v1);
    ...
    public void degreeCounting() throws Exception {
        ...
        for (Map.Entry<Long,Integer> entry : degrees.entrySet()) {
            ...
            JanusVertex v = getV(tx, entry.getKey().longValue());
    ...
    public void degreeCountingDistance() throws Exception {
        ...
        for (JanusGraphComputer.ResultMode mode : JanusGraphComputer.ResultMode.values()) {
            ...
            for (JanusVertex v : gview.query().vertices()) {
                long degree2 = ((Integer)v.value(DegreeCounter.DEGREE)).longValue();
                long actualDegree2 = 0;
                for (JanusVertex w : v.query().direction(Direction.OUT).vertices()) {
            ...
            if (mode== JanusGraphComputer.ResultMode.LOCALTX) {
                assertTrue(gview instanceof JanusTransaction);
                ((JanusTransaction)gview).rollback();
            }
        }
    }
...
//                }
    ...
    public void testPageRank() throws ExecutionException, InterruptedException {
        ...
        Iterator<JanusVertex> iv = tx.query().vertices().iterator();
        while (iv.hasNext()) {
        ...
        while (ranks.hasNext()) {
            ...
            final JanusVertex u = getV(tx, vertexID);
    ...
    public void testShortestDistance() throws Exception {
        ...
        int maxDepth = 16;
        int maxBranch = 5;
        JanusVertex vertex = tx.addVertex();
        ...
        while (distances.hasNext()) {
            ...
            JanusVertex v = getV(tx, kv.getKey());
