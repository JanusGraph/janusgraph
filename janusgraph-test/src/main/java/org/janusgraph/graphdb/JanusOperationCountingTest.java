...
public abstract class JanusOperationCountingTest extends JanusGraphBaseTest {
    ...
    public void testReadOperations(boolean cache) {
        ...
        JanusTransaction tx = graph.buildTransaction().groupName(metricsPrefix).start();
        ...
        tx = graph.buildTransaction().groupName(metricsPrefix).start();
        JanusVertex v = tx.addVertex(), u = tx.addVertex("person");
    ...
    public void testSettingProperty() throws Exception {
        ...
        JanusTransaction tx = graph.buildTransaction().checkExternalVertexExistence(false).groupName(metricsPrefix).start();
...
//        printAllMetrics();
    ...
    public void testKCVSAccess1() throws InterruptedException {
        ...
        JanusTransaction tx = graph.buildTransaction().groupName(metricsPrefix).start();
        JanusVertex v = tx.addVertex("age", 25, "name", "john");
        JanusVertex u = tx.addVertex("age", 35, "name", "mary");
        v.addEdge("knows", u);
        tx.commit();
        verifyStoreMetrics(EDGESTORE_NAME);
        verifyLockingOverwrite(INDEXSTORE_NAME, 3);
    ...
    public void checkPropertyLockingAndIndex() {
        ...
        JanusTransaction tx = graph.buildTransaction().groupName(metricsPrefix).start();
        JanusVertex v = tx.addVertex("uid", "v1", "age", 25, "name", "john");
...
//    }
    ...
    public void testCacheConcurrency() throws InterruptedException {
        ...
        for (int i=0;i<numV;i++) {
            JanusVertex v = graph.addVertex(prop,0);
            graph.tx().commit();
            vids[i]=getId(v);
        }
        clopen(newConfig);
        ...
        Thread updater = new Thread(new Runnable() {
            ...
            public void run() {
                for (int i=0;i<numV;i++) {
                    try {
                        JanusVertex v = getV(graph,vids[i]);
...
//        reader.start();
    ...
    public void testCacheSpeedup() {
        ...
        for (int i=0;i<numV;i++) {
            JanusVertex v = graph.addVertex("name", "v" + i);
            if (previous!=null)
