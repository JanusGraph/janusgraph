...
public abstract class JanusGraphConcurrentTest extends JanusGraphBaseTest {
                ...
                while (run.get()) {
                    ...
                    try {
                        for (int i = 0; i < batchR; i++) {
                            ...
                            Iterable<JanusVertex> vertices = tx.query().has("k",random.nextInt(maxK)).has("q",random.nextInt(maxQ)).vertices();
    ...
    private static class RandomPropertyMaker implements Runnable {
        private final JanusTransaction tx;
        ...
        public void run() {
            while (true) {
                ...
                JanusVertex n = getOnlyVertex(tx.query().has(idKey, RandomGenerator.randomInt(0, nodeCount)));
    ...
    private static class FixedRelationshipMaker implements Runnable {
        ...
        public FixedRelationshipMaker(JanusTransaction tx,
        ...
        public void run() {
            while (true) {
                ...
                JanusVertex source = Iterables.<JanusVertex>getOnlyElement(tx.query().has(idKey, 0).vertices());
                JanusVertex sink = Iterables.<JanusVertex>getOnlyElement(tx.query().has(idKey, 1).vertices());
    ...
    private static class SimpleReader extends BarrierRunnable {
        ...
        public SimpleReader(JanusTransaction tx, CountDownLatch startLatch,
        ...
        protected void doRun() throws Exception {
            JanusVertex v = Iterables.<JanusVertex>getOnlyElement(tx.query().has(idKey, vertexid).vertices());

            for (int i = 0; i < nodeTraversalCount; i++) {
                assertCount(expectedEdges, v.query().labels(label2Traverse).direction(Direction.BOTH).edges());
                for (JanusEdge r : v.query().direction(Direction.OUT).labels(label2Traverse).edges()) {
    ...
    private abstract static class BarrierRunnable implements Runnable {
        ...
        protected final JanusTransaction tx;
        protected final CountDownLatch startLatch;
        protected final CountDownLatch stopLatch;

        public BarrierRunnable(JanusTransaction tx, CountDownLatch startLatch, CountDownLatch stopLatch) {
            this.tx = tx;
