...
public abstract class JanusGraphBaseTest {
    ...
    public JanusTransaction tx;
    public JanusManagement mgmt;
    ...
    public void closeLogManager(String logManagerName) {
        if (logManagers.containsKey(logManagerName)) {
            try {
                ...
                throw new JanusException("Could not close log manager " + logManagerName,e);
            }
        }
    }
    ...
    private Log openLog(String logManagerName, String logName) {
        try {
            ...
            throw new JanusException("Could not open log: "+ logName,e);
        }
    }
    ...
    public JanusGraphIndex getExternalIndex(Class<? extends Element> clazz, String backingIndex) {
        ...
        else if (JanusVertexProperty.class.isAssignableFrom(clazz)) prefix = "p";
    ...
    public static int getThreadCount() {
        String s = System.getProperty("janus.test.threads");
        if (null != s)
    ...
    public JanusVertex getVertex(String key, Object value) {
        return getVertex(tx,key,value);
    }

    public JanusVertex getVertex(PropertyKey key, Object value) {
        return getVertex(tx,key,value);
    }

    public static JanusVertex getVertex(JanusTransaction tx, String key, Object value) {
        return (JanusVertex)getOnlyElement(tx.query().has(key,value).vertices(),null);
    }

    public static JanusVertex getVertex(JanusTransaction tx, PropertyKey key, Object value) {
    ...
    public static JanusVertex getOnlyVertex(JanusGraphQuery<?> query) {
        return (JanusVertex)getOnlyElement(query.vertices());
    }

    public static JanusEdge getOnlyEdge(JanusVertexQuery<?> query) {
        return (JanusEdge)getOnlyElement(query.edges());
    }
...
//    }
    ...
    public static JanusVertex getV(JanusGraphTransaction g, Object vid) {
        if (!g.vertices(vid).hasNext()) return null;
        return (JanusVertex)g.vertices(vid).next();
    }

    public static JanusEdge getE(JanusGraphTransaction g, Object eid) {
        if (!g.edges(eid).hasNext()) return null;
        return (JanusEdge)g.edges(eid).next();
    }
