...
public abstract class JanusGraphBaseTest {
    ...
    public static Map<TestConfigOption,Object> validateConfigOptions(Object... settings) {
        //Parse settings
        Preconditions.checkArgument(settings.length%2==0,"Expected even number of settings: "+settings);
        final Map<TestConfigOption,Object> options = Maps.newHashMap();
        for (int i=0;i<settings.length;i=i+2) {
            Preconditions.checkState(settings[i] instanceof TestConfigOption, "Expected configuration option but got: "+ settings[i]);
            Preconditions.checkNotNull(settings[i+1],"Null setting at position ["+(i+1)+"]");
