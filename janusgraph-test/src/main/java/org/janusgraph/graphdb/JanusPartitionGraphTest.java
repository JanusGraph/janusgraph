 ...
 */
public abstract class JanusPartitionGraphTest extends JanusGraphBaseTest {

    private static final Logger log =
            LoggerFactory.getLogger(JanusPartitionGraphTest.class);
...
//    }
    ...
    public void testVertexPartitioning() throws Exception {
        ...
        for (int i = 0; i < numG; i++) {
            JanusVertex g = tx.addVertex("group");
        ...
        for (int i = 0; i < numG; i++) {
            ...
            JanusVertexProperty p = (JanusVertexProperty) getOnlyElement(g.properties("gid"));
            assertEquals(canonicalPartition, getPartitionID(p));
            for (Iterator<VertexProperty<Object>> niter = g.properties("name"); niter.hasNext(); ) {
                assertEquals(canonicalPartition,getPartitionID((JanusVertex) niter.next().element()));
            }
        ...
        for (int t = 1; t <= numTx; t++) {
            JanusVertex g1 = getV(tx, gids[0]), g2 = getV(tx, gids[1]);
            assertNotNull(g1);
            JanusVertex[] vs = new JanusVertex[vPerTx];
            ...
            JanusTransaction txx = graph.buildTransaction().readOnly().start();
            g1 = getV(tx, gids[0]);
            g2 = getV(tx, gids[1]);
            int partition = -1;
            for (int vi = 0; vi < vPerTx; vi++) {
                ...
                int numRels = 0;
                JanusVertex v = getV(txx, vs[vi].longId());
                for (JanusRelation r : v.query().relations()) {
                    numRels++;
                    assertEquals(partition, getPartitionID(r));
                    if (r instanceof JanusEdge) {
                        JanusVertex o = ((JanusEdge) r).otherVertex(v);
        ...
        for (int t = 0; t < 10; t++) {
            ...
            JanusTransaction tx2 = graph.buildTransaction().restrictedPartitions(partarr).readOnly().start();
            //Copied from above
            g1 = getV(tx2, gids[0]);
            assertEquals(0, g1.<Integer>value("gid").intValue());
            assertEquals("group", g1.label());
            ...
            JanusVertex g2 = getV(tx2, gids[1]);
    ...
    private int setupGroupClusters(int[] groupDegrees, CommitMode commitMode) {
        ...
        int numVertices = 0;
        JanusVertex[] groups = new JanusVertex[groupDegrees.length];
        for (int i = 0; i < groupDegrees.length; i++) {
            ...
            for (int noEdges = 0; noEdges < groupDegrees[i]; noEdges++) {
                JanusVertex g = vInTx(groups[i],tx);
                JanusVertex p = tx.addVertex("name","person"+i+":"+noEdges,"clusterId","group"+i);
                numVertices++;
                p.addEdge("member",g);
                g.addEdge("contain", p);
                if (commitMode==CommitMode.PER_VERTEX) newTx();
    ...
    private static JanusVertex vInTx(JanusVertex v, JanusTransaction tx) {
        if (!v.hasId()) return v;
    ...
    private void testPartitionSpread(boolean flush, boolean batchCommit) {
        ...
        for (int i=0;i<groupDegrees.length;i++) {
            JanusVertex g = getOnlyVertex(tx.query().has("groupid","group"+i));
            ...
            for (JanusVertex v : g.query().direction(Direction.IN).labels("member").vertices()) {
