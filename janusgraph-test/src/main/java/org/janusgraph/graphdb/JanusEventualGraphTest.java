...
import static org.janusgraph.testutil.JanusAssert.assertCount;
...
public abstract class JanusEventualGraphTest extends JanusGraphBaseTest {

    private Logger log = LoggerFactory.getLogger(JanusEventualGraphTest.class);
    ...
    public void concurrentIndexTest() {
        ...
        JanusVertex v = tx.addVertex("uid", "v");
        ...
        assertEquals("v", Iterators.<String>getOnlyElement(Iterables.<JanusVertex>getOnlyElement(tx.query().has("value", 11).vertices()).values("uid")));
    }
    ...
    public void testTimestampSetting() {
        ...
        JanusTransaction tx1 = graph.buildTransaction().commitTime(Instant.ofEpochSecond(100)).start();
        String name = "name";
        String age = "age";
        String address = "address";

        JanusVertex v1 = tx1.addVertex(name, "a");
        JanusVertex v2 = tx1.addVertex(age, "14", name, "b", age, "42");
        tx1.commit();
        ...
        // an edge v2 -> v1
        JanusTransaction tx2 = graph.buildTransaction().commitTime(Instant.ofEpochSecond(1000)).start();
        v1 = getV(tx2,id1);
        v2 = getV(tx2,id2);
        ...
        JanusVertex afterTx2 = getV(graph,id1);
        ...
        JanusTransaction tx3 = graph.buildTransaction().commitTime(Instant.ofEpochSecond(200)).start();
        v1 = getV(tx3,id1);
        v1.property(address).remove();
        tx3.commit();

        JanusVertex afterTx3 = getV(graph,id1);
        graph.tx().commit();
        ...
        JanusTransaction tx4 = graph.buildTransaction().commitTime(Instant.ofEpochSecond(2000)).start();
        v2 = getV(tx4,id2);
        v2.property(VertexProperty.Cardinality.single, age,  "15");
        getE(tx4,edgeId).remove();
        tx4.commit();

        JanusVertex afterTx4 = getV(graph,id2);
        ...
        JanusTransaction tx5 = graph.buildTransaction().commitTime(Instant.ofEpochSecond(1500)).start();
        v2 = getV(tx5,id2);
        v2.property(VertexProperty.Cardinality.single, age,  "16");
        tx5.commit();
        JanusVertex afterTx5 = getV(graph,id2);
    ...
    public void testTimestampedEdgeUpdates() {
        ...
        JanusTransaction tx = graph.buildTransaction().commitTime(Instant.ofEpochSecond(100)).start();
    ...
    public void testBatchLoadingLocking(boolean batchloading) {
        ...
        for (int i=0;i<numV;i++) {
            JanusVertex v = tx.addVertex("uid",i+1);
            v.addEdge("knows",v);
        }
        clopen();
//        System.out.println("Time: " + (System.currentTimeMillis()-start));
        ...
        for (int i=0;i<Math.min(numV,300);i++) {
            ...
            JanusVertex v = Iterables.<JanusVertex>getOnlyElement(graph.query().has("uid", i + 1).vertices());
    ...
    public void testConsistencyModifier() throws InterruptedException {
        ...
        JanusVertex u = tx.addVertex(), v = tx.addVertex();
        JanusRelation[] rs = new JanusRelation[9];
        final int txid = 1;
    ...
    private void processTx(JanusTransaction tx, int txid, long vid, long uid) {
        JanusVertex v = getV(tx,vid);
        JanusVertex u = getV(tx,uid);
        ...
        for (String pkey : new String[]{"value","valuef"}) {
            ...
            sign((JanusVertexProperty)p,txid);
        }
    ...
    private JanusRelation sign(JanusRelation r, int id) {
        r.property("sig",id);
        return r;
    }
