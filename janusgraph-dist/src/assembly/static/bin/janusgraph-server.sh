...
if [[ -z "$JANUSGRAPH_YAML" ]]; then
  JANUSGRAPH_YAML=$JANUSGRAPH_CONF/janusgraph-server.yaml
fi
...
if [[ -z "$JAVA_OPTIONS_FILE" ]]; then
  ...
  do
    JAVA_OPTIONS="$JAVA_OPTIONS $opt"
  done
fi

JAVA_OPTIONS="$JAVA_OPTIONS -javaagent:$JANUSGRAPH_LIB/jamm-0.3.0.jar"
...
case "$1" in
  ...
  *)
    ...
    echo
    echo "    start        Start the server in the background using conf/janusgraph-server.yaml as the"
    echo "                 default configuration file"
    echo "    stop         Stop the server"
    echo "    restart      Stop and start the server"
    echo "    status       Check if the server is running"
    echo "    console      Start the server in the foreground using conf/janusgraph-server.yaml as the"
    echo "                 default configuration file"
    echo
