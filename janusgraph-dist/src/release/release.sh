...
pattern='janus\.compatible.versions'
POM_FILE=pom.xml
...
echo "Current value of janus.compatible.versions:"
echo "  $compat"
...
if [ "$line" != "n" ]; then
    ...
    if echo "$compat" | grep '<janus\.compatible\.versions[ 	]*/>' >/dev/null; then
        compat=$(echo "$compat" | sed -r 's#^(.*janus\.compatible\.versions).+$#\1>'"$ver"'</janus.compatible.versions>#')
    else
        compat=$(echo "$compat" | sed -r 's#^(.+>)(.+)(<.+)$#\1\2,'"$ver"'\3#' )
    fi
    echo "New value of janus.compatible.versions:"
    echo "  $compat"
    require_yes
    echo "Generating $POM_FILE.new with updated janus.compatible.versions..."
    cat "$POM_FILE" | sed -r 's#^.*<janus\.compatible\.versions.*$#'"$compat"'#' > "${POM_FILE}.new"
    ...
    echo "Saved new janus.compatible.versions value to $POM_FILE"
    echo "Committing changes to ${POM_FILE}..."
    git add "${POM_FILE}" 
    git commit -m 'Updating janus.compatible.versions prop' "${POM_FILE}"
fi
...
janus-dist/src/release/rewrite-tag.sh release.properties

# Add the release to the list used in forward-compatibility testing
# during subsequent releases.
janus-dist/src/release/commit-compat-version.sh release.properties
...
janus-dist/target/release/gh-pages-update.sh release.properties
