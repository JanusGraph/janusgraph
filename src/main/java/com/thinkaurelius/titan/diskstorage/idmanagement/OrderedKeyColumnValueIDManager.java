...
public class OrderedKeyColumnValueIDManager extends AbstractIDManager {
	...
	public OrderedKeyColumnValueIDManager(KeyColumnValueStoreManager manager, Configuration config) throws StorageException {
        super(config);
    ...
    private long getNextID(ByteBuffer partitionKey, long blockSize, StoreTransactionHandle txh) throws StorageException {
        List<Entry> blocks = store.getSlice(partitionKey, EMPTY_BUFFER, EMPTY_BUFFER, 5, txh);
		...
		for (int retry = 0; retry < idApplicationRetryCount; retry++) {
            ...
            try {
                ...
                try {
                    ...
                    store.mutate(partitionKey, Arrays.asList(new Entry(target, EMPTY_BUFFER)), null, null);
                    ...
                    if (idApplicationWaitMS < after - before) {
                        ...
                        List<Entry> blocks = store.getSlice(partitionKey, slice[0], slice[1], null);
                        if (blocks==null) throw new TemporaryStorageException("Could not read from storage");
