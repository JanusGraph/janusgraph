 ...
 * 
 * A VirtualEntity does not keep track of the type of edges being loaded which will be inefficient
 * in any actual implementation. Use one of its child classes to get this functionality.
 * 
 * @author matthias
 *
 */
public class VirtualEntity implements InternalEntity {
	
	private byte lifecycle;
	
	public VirtualEntity(boolean isNew) {
		if (isNew)
			lifecycle = New;
		else 
			lifecycle = Loaded;
	...
	VirtualEntity(VirtualEntity clone) {
	...
	public final boolean modified() {
		switch(lifecycle) {
		case Deleted: throw new IllegalStateException("Entity is already deleted");
