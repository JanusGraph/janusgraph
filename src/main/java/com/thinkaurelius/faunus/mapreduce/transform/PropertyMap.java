            ...
            if (this.isVertex) {
                if (value.hasPaths()) {
                    for (int i = 0; i < value.pathCount(); i++) {
                        context.write(NullWritable.get(), this.handler.set(ElementPicker.getProperty(value, this.key)));
                    }
                ...
                for (final Edge e : value.getEdges(Direction.OUT)) {
                    ...
                    if (edge.hasPaths()) {
                        for (int i = 0; i < edge.pathCount(); i++) {
                            context.write(NullWritable.get(), this.handler.set(ElementPicker.getProperty(edge, this.key)));
                        }
                        edgesProcessed++;
                    }
                }
