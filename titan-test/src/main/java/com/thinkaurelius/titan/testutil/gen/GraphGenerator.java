...
public class GraphGenerator {
     ...
     * 
     * @param g
     *            A graph with types predefined. If it already contains vertices
    ...
    private final EdgeAnnotator edgeAnnotator = new EdgeAnnotator() {
         ...
         * keys from consideration. These keys are used by primary keys.
         * <p>
         * In addition to the preceding, lookup the primary key for the edge's
         * label and set that to a randomly chosen value.
         */
        @Override
        public void annotate(Edge edge) {
            // Set primary key edge property
            edge.setProperty(schema.getPrimaryKeyForLabel(edge.getLabel()),
