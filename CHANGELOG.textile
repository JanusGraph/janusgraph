...
* Better internal memory representation for sequential memory access and smaller memory footprint
* Replaced Kryo with a custom serializer for better performance, using Kryo only for complex objects
* Introduced a compressed String attribute called StringX which has a much smaller storage footprint than String.
* Introduced native serialization for primitive arrays
...
* Added TTL (time-to-live) support at the diskstorage and graph layers with the Cassandra backend
