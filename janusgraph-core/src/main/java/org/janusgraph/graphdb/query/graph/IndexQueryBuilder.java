...
/**
 * Implementation of {@link JanusIndexQuery} for string based queries that are issued directly against the specified
 ...
 */
public class IndexQueryBuilder extends BaseQuery implements JanusIndexQuery {
    ...
    public IndexQueryBuilder(StandardJanusTx tx, IndexSerializer serializer) {
    ...
    private Iterable<Result<JanusElement>> execute(ElementCategory resultType) {
        ...
        final Function<Object, ? extends JanusElement> conversionFct = tx.getConversionFunction(resultType);
        return Iterables.filter(Iterables.transform(result, new Function<RawQuery.Result, Result<JanusElement>>() {
            @Nullable
            @Override
            public Result<JanusElement> apply(@Nullable RawQuery.Result result) {
                return new ResultImpl<JanusElement>(conversionFct.apply(result.getResult()),result.getScore());
            }
        }),new Predicate<Result<JanusElement>>() {
            @Override
            public boolean apply(@Nullable Result<JanusElement> r) {
