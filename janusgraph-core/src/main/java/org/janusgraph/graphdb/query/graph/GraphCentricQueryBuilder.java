...
public class GraphCentricQueryBuilder implements JanusGraphQuery<GraphCentricQueryBuilder> {
     ...
     */
    private List<PredicateCondition<String, JanusElement>> constraints;
    /**
    ...
    public GraphCentricQueryBuilder(StandardJanusTx tx, IndexSerializer serializer) {
        Preconditions.checkNotNull(tx);
        Preconditions.checkNotNull(serializer);
        this.tx = tx;
        this.serializer = serializer;
        this.constraints = new ArrayList<PredicateCondition<String, JanusElement>>(5);
    }
    ...
    @Override
    public GraphCentricQueryBuilder has(String key, JanusPredicate predicate, Object condition) {
        Preconditions.checkNotNull(key);
        Preconditions.checkNotNull(predicate);
        Preconditions.checkArgument(predicate.isValidCondition(condition), "Invalid condition: %s", condition);
        constraints.add(new PredicateCondition<String, JanusElement>(key, predicate, condition));
        return this;
    }

    public GraphCentricQueryBuilder has(PropertyKey key, JanusPredicate predicate, Object condition) {
        Preconditions.checkNotNull(key);
    ...
    public Iterable<JanusVertex> vertices() {
        ...
        return Iterables.filter(new QueryProcessor<GraphCentricQuery, JanusElement, JointIndexQuery>(query, tx.elementProcessor), JanusVertex.class);
    }

    @Override
    public Iterable<JanusEdge> edges() {
        GraphCentricQuery query = constructQuery(ElementCategory.EDGE);
        return Iterables.filter(new QueryProcessor<GraphCentricQuery, JanusElement, JointIndexQuery>(query, tx.elementProcessor), JanusEdge.class);
    }

    @Override
    public Iterable<JanusVertexProperty> properties() {
        GraphCentricQuery query = constructQuery(ElementCategory.PROPERTY);
        return Iterables.filter(new QueryProcessor<GraphCentricQuery, JanusElement, JointIndexQuery>(query, tx.elementProcessor), JanusVertexProperty.class);
    }
    ...
    public GraphCentricQuery constructQueryWithoutProfile(final ElementCategory resultType) {
        ...
        //Prepare constraints
        And<JanusElement> conditions = QueryUtil.constraints2QNF(tx, constraints);
        ...
        ConditionUtil.traversal(conditions,new Predicate<Condition<JanusElement>>() {
            @Override
            public boolean apply(@Nullable Condition<JanusElement> condition) {
                if (condition instanceof PredicateCondition) {
                    RelationType type = ((PredicateCondition<RelationType,JanusElement>)condition).getKey();
        ...
        while (true) {
            ...
            for (IndexType index : indexCandidates) {
                ...
                if (index.hasSchemaTypeConstraint()) {
                    JanusSchemaType type = index.getSchemaTypeConstraint();
    ...
    public static List<Object[]> indexCover(final CompositeIndexType index, Condition<JanusElement> condition, Set<Condition> covered) {
    ...
    private static void constructIndexCover(Object[] indexValues, int position, IndexField[] fields,
                                            Condition<JanusElement> condition,
    ...
    private static final Map.Entry<Condition,Collection<Object>> getEqualityConditionValues(Condition<JanusElement> condition, RelationType type) {
        for (Condition c : condition.getChildren()) {
            if (c instanceof Or) {
                ...
                PredicateCondition<RelationType, JanusRelation> atom = (PredicateCondition)c;
    ...
    public static final Condition<JanusElement> indexCover(final MixedIndexType index, Condition<JanusElement> condition,
                                                           final IndexSerializer indexInfo, final Set<Condition> covered) {
        assert QueryUtil.isQueryNormalForm(condition);
        assert condition instanceof And;
        And<JanusElement> subcondition = new And<JanusElement>(condition.numChildren());
        for (Condition<JanusElement> subclause : condition.getChildren()) {
    ...
    private static final boolean coversAll(final MixedIndexType index, Condition<JanusElement> condition, IndexSerializer indexInfo) {
        if (condition.getType()==Condition.Type.LITERAL) {
            if (!(condition instanceof  PredicateCondition)) return false;
            PredicateCondition<RelationType, JanusElement> atom = (PredicateCondition) condition;
            ...
            for (Condition<JanusElement> child : condition.getChildren()) {
