...
/**
 * A {@link Comparator} for {@link JanusRelation} that uses a defined order to compare the relations with
...
public class RelationComparator implements Comparator<InternalRelation> {
    ...
    public int compare(final InternalRelation r1, final InternalRelation r2) {
        ...
        if (r1.isProperty()) {
            Object o1 = ((JanusVertexProperty) r1).value();
            Object o2 = ((JanusVertexProperty) r2).value();
    ...
    private int compareOnKey(JanusRelation r1, JanusRelation r2, long typeid, Order order) {
        return compareOnKey(r1,r2,tx.getExistingPropertyKey(typeid),order);
    }

    private int compareOnKey(JanusRelation r1, JanusRelation r2, PropertyKey type, Order order) {
