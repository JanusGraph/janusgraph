...
import org.janusgraph.graphdb.tinkerpop.JanusFeatures;
...
public class StandardJanusGraph extends JanusBlueprintsGraph {
    ...
    static {
        TraversalStrategies graphStrategies = TraversalStrategies.GlobalCache.getStrategies(Graph.class).clone()
                .addStrategies(AdjacentVertexFilterOptimizerStrategy.instance(), JanusLocalQueryOptimizerStrategy.instance(), JanusGraphStepStrategy.instance());

        //Register with cache
        TraversalStrategies.GlobalCache.registerStrategies(StandardJanusGraph.class, graphStrategies);
        TraversalStrategies.GlobalCache.registerStrategies(StandardJanusTx.class, graphStrategies);
    }
    ...
    private Set<StandardJanusTx> openTransactions;
    ...
    public StandardJanusGraph(GraphDatabaseConfiguration configuration) {
        ...
        openTransactions = Collections.newSetFromMap(new ConcurrentHashMap<StandardJanusTx, Boolean>(100, 0.75f, 1));
        ...
        if (globalConfig.has(REGISTRATION_TIME, uniqueInstanceId)) {
            throw new JanusException(String.format("A Janus graph with the same instance id [%s] is already open. Might required forced shutdown.", uniqueInstanceId));
        }
    ...
    @Override
    public synchronized void close() throws JanusException {
        try {
            closeInternal();
        } finally {
            removeHook();
    ...
    private synchronized void closeInternal() {
        ...
        Map<JanusTransaction, RuntimeException> txCloseExceptions = new HashMap<>();
        ...
        try {
             ...
             */
            for (StandardJanusTx otx : openTransactions) {
                try {
                    otx.close();
    ...
    public Features features() {
        return JanusFeatures.getFeatures(this, backend.getStoreFeatures());
    }
...
//    }
    ...
    public Set<? extends JanusTransaction> getOpenTransactions() {
    ...
    @Override
    public JanusTransaction newThreadBoundTransaction() {
        return buildTransaction().threadBound().start();
    }

    public StandardJanusTx newTransaction(final TransactionConfiguration configuration) {
        if (!isOpen) ExceptionFactory.graphShutdown();
        try {
            StandardJanusTx tx = new StandardJanusTx(this, configuration);
            tx.setBackendTransaction(openBackendTransaction(tx));
            openTransactions.add(tx);
            return tx;
        } catch (BackendException e) {
            throw new JanusException("Could not start new transaction", e);
        }
    }

    private BackendTransaction openBackendTransaction(StandardJanusTx tx) throws BackendException {
    ...
    public void closeTransaction(StandardJanusTx tx) {
    ...
    private final SchemaCache.StoreRetrieval typeCacheRetrieval = new SchemaCache.StoreRetrieval() {
        ...
        public Long retrieveSchemaByName(String typeName) {
            ...
            StandardJanusTx consistentTx = null;
            try {
                ...
                JanusVertex v = Iterables.getOnlyElement(QueryUtil.getVertices(consistentTx, BaseKey.SchemaName, typeName), null);
    ...
    public ModificationSummary prepareCommit(final Collection<InternalRelation> addedRelations,
                                     ...
                                     final BackendTransaction mutator, final StandardJanusTx tx,
    ...
    private static final Predicate<InternalRelation> SCHEMA_FILTER = new Predicate<InternalRelation>() {
        ...
        public boolean apply(@Nullable InternalRelation internalRelation) {
            return internalRelation.getType() instanceof BaseRelationType && internalRelation.getVertex(0) instanceof JanusSchemaVertex;
        }
    };
    ...
    public void commit(final Collection<InternalRelation> addedRelations,
                     final Collection<InternalRelation> deletedRelations, final StandardJanusTx tx) {
        ...
        try {
            ...
            else throw new JanusException("Unexpected exception",e);
        }
    }
