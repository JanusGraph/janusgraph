 ...
 */
public class ManagementLoggerImpl implements ManagementLogger {

    private static final Logger log =
            LoggerFactory.getLogger(ManagementLogger.class);

    public static final Duration SLEEP_INTERVAL = Duration.ofMillis(100L);
    public static final Duration MAX_WAIT_TIME = Duration.ofSeconds(60L);
    ...
    public ManagementLoggerImpl(StandardJanusGraph graph, Log sysLog, SchemaCache schemaCache, TimestampProvider times) {
        this.graph = graph;
    ...
    public void read(Message message) {
        ...
        switch (logType) {
            case CACHED_TYPE_EVICTION: {
                ...
                final Thread ack = new Thread(new SendAckOnTxClose(evictionId, senderId, graph.getOpenTransactions(), graph.getGraphName()));
                ack.setDaemon(true);
                ack.start();
                break;
            }
