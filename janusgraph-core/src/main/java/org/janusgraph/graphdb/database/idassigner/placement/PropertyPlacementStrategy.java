...
import org.janusgraph.core.JanusProperty;
...
public class PropertyPlacementStrategy extends SimpleBulkPlacementStrategy {
    ...
    public int getPartition(InternalElement element) {
        if (element instanceof JanusVertex) {
            int pid = getPartitionIDbyKey((JanusVertex)element);
            if (pid>=0) return pid;
        }
    ...
    private int getPartitionIDbyKey(JanusVertex vertex) {
        ...
        JanusVertexProperty p = (JanusVertexProperty)Iterables.getFirst(vertex.query().keys(key).properties(),null);
        if (p==null) return -1;
