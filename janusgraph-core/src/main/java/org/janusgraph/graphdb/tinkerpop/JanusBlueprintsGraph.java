 ...
 */
public abstract class JanusBlueprintsGraph implements JanusGraph {

    private static final Logger log =
            LoggerFactory.getLogger(JanusBlueprintsGraph.class);
    ...
    private ThreadLocal<JanusBlueprintsTransaction> txs = new ThreadLocal<JanusBlueprintsTransaction>() {

        protected JanusBlueprintsTransaction initialValue() {
            return null;
        }

    };

    public abstract JanusTransaction newThreadBoundTransaction();

    private JanusBlueprintsTransaction getAutoStartTx() {
        if (txs == null) throw new IllegalStateException("Graph has been closed");
        tinkerpopTxContainer.readWrite();

        JanusBlueprintsTransaction tx = txs.get();
        Preconditions.checkState(tx!=null,"Invalid read-write behavior configured: " +
                "Should either open transaction or throw exception.");
        return tx;
    }

    private JanusBlueprintsTransaction startNewTx() {
        JanusBlueprintsTransaction tx = txs.get();
        if (tx!=null && tx.isOpen()) throw Transaction.Exceptions.transactionAlreadyOpen();
        tx = (JanusBlueprintsTransaction) newThreadBoundTransaction();
        txs.set(tx);
        log.debug("Created new thread-bound transaction {}", tx);
        return tx;
    }

    public JanusTransaction getCurrentThreadTx() {
    ...
    public <I extends Io> I io(final Io.Builder<I> builder) {
        return (I) builder.graph(this).registry(JanusIoRegistry.getInstance()).create();
    }
...
//    }
    ...
    class GraphTransaction extends AbstractThreadLocalTransaction {
        ...
        public GraphTransaction() {
            super(JanusBlueprintsGraph.this);
        }
        ...
        @Override
        public JanusTransaction createThreadedTx() {
