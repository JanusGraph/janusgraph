...
public class DropStepBatchFetcher extends MultiQueriableStepBatchFetcher<Iterable<JanusGraphRelation>>{
    ...
    public DropStepBatchFetcher(FetchQueryBuildFunction fetchQueryBuildFunction, int batchSize,
                                TriggeredBatchConsumer<JanusGraphVertex> batchConsumerBeforeQuery) {
        super(batchSize, batchConsumerBeforeQuery);
        this.fetchQueryBuildFunction = fetchQueryBuildFunction;
    }

    @Override
    protected Map<JanusGraphVertex, Iterable<JanusGraphRelation>> makeQueryAndExecute(JanusGraphMultiVertexQuery multiQuery) {
        multiQuery = fetchQueryBuildFunction.makeQuery(multiQuery);
        return multiQuery.drop();
    }
}
