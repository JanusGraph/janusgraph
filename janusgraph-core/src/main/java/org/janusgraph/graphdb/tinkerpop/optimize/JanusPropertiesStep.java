 ...
 */
public class JanusPropertiesStep<E> extends PropertiesStep<E> implements HasStepFolder<Element, E>, Profiling, MultiQueriable<Element,E> {

    public JanusPropertiesStep(PropertiesStep<E> originalStep) {
    ...
    private Map<JanusVertex, Iterable<? extends JanusProperty>> multiQueryResults = null;
    ...
    private Iterator<E> convertIterator(Iterable<? extends JanusProperty> iterable) {
        if (getReturnType().forProperties()) return (Iterator<E>) iterable.iterator();
        assert getReturnType().forValues();
        return (Iterator<E>) Iterators.transform(iterable.iterator(), p -> ((JanusProperty) p).value());
    }
    ...
    @Override
    public JanusPropertiesStep<E> clone() {
        final JanusPropertiesStep<E> clone = (JanusPropertiesStep<E>) super.clone();
