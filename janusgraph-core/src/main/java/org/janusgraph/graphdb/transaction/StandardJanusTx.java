...
import org.janusgraph.graphdb.tinkerpop.JanusBlueprintsTransaction;
...
public class StandardJanusTx extends JanusBlueprintsTransaction implements TypeInspector, SchemaInspector, VertexFactory {

    private static final Logger log = LoggerFactory.getLogger(StandardJanusTx.class);
     ...
     * of thread parallelism can lead to Janus generating multiple copies of
    ...
    /**
     * Caches Janus types by name so that they can be quickly retrieved once they are loaded in the transaction.
    ...
    public StandardJanusTx(StandardJanusGraph graph, TransactionConfiguration config) {
...
//        this.idInspector = idManager.getIdInspector();
        ...
        if (null != config.getGroupName()) {
            ...
            elementProcessor = new MetricsQueryExecutor<GraphCentricQuery, JanusElement, JointIndexQuery>(config.getGroupName(), "graph", elementProcessorImpl);
            edgeProcessor    = new MetricsQueryExecutor<VertexCentricQuery, JanusRelation, SliceQuery>(config.getGroupName(), "vertex", edgeProcessorImpl);
        } else {
    ...
    private void verifyWriteAccess(JanusVertex... vertices) {
        if (config.isReadOnly())
    ...
    public final void verifyAccess(JanusVertex... vertices) {
        verifyOpen();
        for (JanusVertex v : vertices) {
    ...
    public StandardJanusTx getNextTx() {
        ...
        else return (StandardJanusTx) graph.getCurrentThreadTx();
    }
    ...
    public boolean isPartitionedVertex(JanusVertex vertex) {
    ...
    public InternalVertex getOtherPartitionVertex(JanusVertex partitionedVertex, long otherPartition) {
        Preconditions.checkArgument(isPartitionedVertex(partitionedVertex));
        return getExistingVertex(idManager.getPartitionedVertexId(partitionedVertex.longId(), otherPartition));
    }

    public InternalVertex[] getAllRepresentatives(JanusVertex partitionedVertex, boolean restrict2Partitions) {
        Preconditions.checkArgument(isPartitionedVertex(partitionedVertex));
        long[] ids;
    ...
    @Override
    public Iterable<JanusVertex> getVertices(long... ids) {
        verifyOpen();
        ...
        List<JanusVertex> result = new ArrayList<JanusVertex>(ids.length);
        ...
        for (Iterator<JanusVertex> iterator = result.iterator(); iterator.hasNext(); ) {
            ...
            if (idInspector.isRelationTypeId(vertexid)) {
                if (idInspector.isPropertyKeyId(vertexid)) {
                    if (idInspector.isSystemRelationTypeId(vertexid)) {
                        ...
                        vertex = new PropertyKeyVertex(StandardJanusTx.this, vertexid, lifecycle);
                    }
                } else {
                    ...
                    if (idInspector.isSystemRelationTypeId(vertexid)) {
                        ...
                        vertex = new EdgeLabelVertex(StandardJanusTx.this, vertexid, lifecycle);
                    }
                }
            } else if (idInspector.isVertexLabelVertexId(vertexid)) {
                vertex = new VertexLabelVertex(StandardJanusTx.this,vertexid, lifecycle);
            } else if (idInspector.isGenericSchemaVertexId(vertexid)) {
                vertex = new JanusSchemaVertex(StandardJanusTx.this,vertexid, lifecycle);
            } else if (idInspector.isUserVertexId(vertexid)) {
                if (createStubVertex) vertex = new PreloadedVertex(StandardJanusTx.this, vertexid, lifecycle);
                else vertex = new CacheVertex(StandardJanusTx.this, vertexid, lifecycle);
    ...
    @Override
    public JanusVertex addVertex(Long vertexId, VertexLabel label) {
        verifyWriteAccess();
    ...
    public JanusVertex addVertex(VertexLabel vertexLabel) {
    ...
    private Iterable<InternalVertex> getInternalVertices() {
        ...
        if (!addedRelations.isEmpty()) {
            ...
            while (viter.hasNext()) {
                if (viter.next() instanceof JanusSchemaElement) viter.remove();
            }
    ...
    public void removeRelation(InternalRelation relation) {
        ...
        if (relation.isNew()) {
            ...
            if (TypeUtil.hasSimpleInternalVertexKeyIndex(relation)) newVertexIndexEntries.remove((JanusVertexProperty) relation);
        } else {
    ...
    private TransactionLock getUniquenessLock(final JanusVertex out, final InternalRelationType type, final Object in) {
    ...
    public JanusEdge addEdge(JanusVertex outVertex, JanusVertex inVertex, EdgeLabel label) {
    ...
    private void connectRelation(InternalRelation r) {
        ...
        if (TypeUtil.hasSimpleInternalVertexKeyIndex(r)) newVertexIndexEntries.add((JanusVertexProperty) r);
    }

    public JanusVertexProperty addProperty(JanusVertex vertex, PropertyKey key, Object value) {
        return addProperty(key.cardinality().convert(), vertex, key, value);
    }

    public JanusVertexProperty addProperty(VertexProperty.Cardinality cardi, JanusVertex vertex, PropertyKey key, Object value) {
...
//                    if (!Iterables.isEmpty(Iterables.filter(query(vertex).type(key).properties(), new Predicate<JanusVertexProperty>() {
//                        @Override
//                        public boolean apply(@Nullable JanusVertexProperty janusProperty) {
//                            return normalizedValue.equals(janusProperty.value());
//                        }
//                    })))
...
//            }
            ...
            if (cardi==VertexProperty.Cardinality.single || cardi== VertexProperty.Cardinality.set) {
                Consumer<JanusRelation> propertyRemover;
                if (cardi==VertexProperty.Cardinality.single)
                    propertyRemover = p -> p.remove();
                else
                    propertyRemover = p -> { if (((JanusVertexProperty)p).value().equals(normalizedValue)) p.remove(); };
    ...
    @Override
    public Iterable<JanusEdge> getEdges(RelationIdentifier... ids) {
        verifyOpen();
        ...
        List<JanusEdge> result = new ArrayList<>(ids.length);
        for (RelationIdentifier id : ids) {
            if (id==null) continue;
            JanusEdge edge = id.findEdge(this);
    ...
    public final JanusSchemaVertex makeSchemaVertex(JanusSchemaCategory schemaCategory, String name, TypeDefinitionMap definition) {
        verifyOpen();
        ...
        JanusSchemaVertex schemaVertex;
        if (schemaCategory.isRelationType()) {
            if (schemaCategory == JanusSchemaCategory.PROPERTYKEY) {
                ...
                assert schemaCategory == JanusSchemaCategory.EDGELABEL;
        ...
        } else if (schemaCategory==JanusSchemaCategory.VERTEXLABEL) {
            ...
            schemaVertex = new JanusSchemaVertex(this, IDManager.getTemporaryVertexID(IDManager.VertexIDType.GenericSchemaType,temporaryIds.nextID()), ElementLifeCycle.New);
        }
        ...
        for (Map.Entry<TypeDefinitionCategory,Object> def : definition.entrySet()) {
            JanusVertexProperty p = addProperty(schemaVertex, BaseKey.SchemaDefinitionProperty, def.getValue());
    ...
    public void updateSchemaVertex(JanusSchemaVertex schemaVertex) {
    ...
    public PropertyKey makePropertyKey(String name, TypeDefinitionMap definition) {
        return (PropertyKey) makeSchemaVertex(JanusSchemaCategory.PROPERTYKEY, name, definition);
    }

    public EdgeLabel makeEdgeLabel(String name, TypeDefinitionMap definition) {
        return (EdgeLabel) makeSchemaVertex(JanusSchemaCategory.EDGELABEL, name, definition);
    }

    public JanusSchemaVertex getSchemaVertex(String schemaName) {
        ...
        if (schemaId != null) {
            ...
            return (JanusSchemaVertex)typeVertex;
        } else return null;
    }
    ...
    public RelationType getRelationType(String name) {
        ...
        return (RelationType)getSchemaVertex(JanusSchemaCategory.getRelationTypeName(name));
    }
    ...
    public boolean containsVertexLabel(String name) {
        ...
        return getSchemaVertex(JanusSchemaCategory.VERTEXLABEL.getSchemaName(name))!=null;
    }
    ...
    public VertexLabel getVertexLabel(String name) {
        ...
        return (VertexLabel)getSchemaVertex(JanusSchemaCategory.VERTEXLABEL.getSchemaName(name));
    }
    ...
    public VertexCentricQueryBuilder query(JanusVertex vertex) {
        return new VertexCentricQueryBuilder(((InternalVertex) vertex).it());
    }

    @Override
    @Deprecated
    public JanusMultiVertexQuery multiQuery(JanusVertex... vertices) {
        MultiVertexCentricQueryBuilder builder = new MultiVertexCentricQueryBuilder(this);
        for (JanusVertex v : vertices) builder.addVertex(v);
        return builder;
    }

    @Override
    @Deprecated
    public JanusMultiVertexQuery multiQuery(Collection<JanusVertex> vertices) {
        MultiVertexCentricQueryBuilder builder = new MultiVertexCentricQueryBuilder(this);
    ...
    public final QueryExecutor<VertexCentricQuery, JanusRelation, SliceQuery> edgeProcessor;

    public final QueryExecutor<VertexCentricQuery, JanusRelation, SliceQuery> edgeProcessorImpl = new QueryExecutor<VertexCentricQuery, JanusRelation, SliceQuery>() {
        @Override
        public Iterator<JanusRelation> getNew(final VertexCentricQuery query) {
            ...
            if (vertex.isNew() || vertex.hasAddedRelations()) {
                return (Iterator) vertex.getAddedRelations(new Predicate<InternalRelation>() {
                    ...
                    private JanusRelation previous = null;

                    @Override
                    public boolean apply(@Nullable InternalRelation relation) {
                        if ((relation instanceof JanusEdge) && relation.isLoop()
        ...
        @Override
        public boolean isDeleted(VertexCentricQuery query, JanusRelation result) {
        ...
        @Override
        public Iterator<JanusRelation> execute(final VertexCentricQuery query, final SliceQuery sq, final Object exeInfo, final QueryProfiler profiler) {
            assert exeInfo==null;
            ...
            return RelationConstructor.readRelation(v, iter, StandardJanusTx.this).iterator();
        }
    };

    public final QueryExecutor<GraphCentricQuery, JanusElement, JointIndexQuery> elementProcessor;

    public final QueryExecutor<GraphCentricQuery, JanusElement, JointIndexQuery> elementProcessorImpl = new QueryExecutor<GraphCentricQuery, JanusElement, JointIndexQuery>() {

        private PredicateCondition<PropertyKey, JanusElement> getEqualityCondition(Condition<JanusElement> condition) {
            if (condition instanceof PredicateCondition) {
                PredicateCondition<PropertyKey, JanusElement> pc = (PredicateCondition) condition;
                if (pc.getPredicate() == Cmp.EQUAL && TypeUtil.hasSimpleInternalVertexKeyIndex(pc.getKey())) return pc;
            } else if (condition instanceof And) {
                for (Condition<JanusElement> child : ((And<JanusElement>) condition).getChildren()) {
                    PredicateCondition<PropertyKey, JanusElement> p = getEqualityCondition(child);
                    if (p != null) return p;
                }
            }
            return null;
        ...
        @Override
        public Iterator<JanusElement> getNew(final GraphCentricQuery query) {
            ...
            if (query.getResultType() == ElementCategory.VERTEX && hasModifications()) {
                ...
                PredicateCondition<PropertyKey, JanusElement> standardIndexKey = getEqualityCondition(query.getCondition());
                Iterator<JanusVertex> vertices;
                if (standardIndexKey == null) {
                    final Set<PropertyKey> keys = Sets.newHashSet();
                    ConditionUtil.traversal(query.getCondition(), new Predicate<Condition<JanusElement>>() {
                        @Override
                        public boolean apply(@Nullable Condition<JanusElement> cond) {
                            ...
                            if (cond instanceof PredicateCondition)
                                keys.add(((PredicateCondition<PropertyKey, JanusElement>) cond).getKey());
                            return true;
                        }
                    });
                    Preconditions.checkArgument(!keys.isEmpty(), "Invalid query condition: %s", query.getCondition());
                    Set<JanusVertex> vertexSet = Sets.newHashSet();
                    for (JanusRelation r : addedRelations.getView(new Predicate<InternalRelation>() {
                        @Override
                        public boolean apply(@Nullable InternalRelation relation) {
                            return keys.contains(relation.getType());
                        }
                    })) {
                        vertexSet.add(((JanusVertexProperty) r).element());
                    }
                    for (JanusRelation r : deletedRelations.values()) {
                        if (keys.contains(r.getType())) {
                            JanusVertex v = ((JanusVertexProperty) r).element();
                            if (!v.isRemoved()) vertexSet.add(v);
                        }
                    }
                    vertices = vertexSet.iterator();
                } else {
                    vertices = com.google.common.collect.Iterators.transform(newVertexIndexEntries.get(standardIndexKey.getValue(), standardIndexKey.getKey()).iterator(), new Function<JanusVertexProperty, JanusVertex>() {
                        @Nullable
                        @Override
                        public JanusVertex apply(@Nullable JanusVertexProperty o) {
                            return o.element();
                        }
                    });
                }

                return (Iterator) com.google.common.collect.Iterators.filter(vertices, new Predicate<JanusVertex>() {
                    @Override
                    public boolean apply(@Nullable JanusVertex vertex) {
        ...
        @Override
        public boolean isDeleted(GraphCentricQuery query, JanusElement result) {
        ...
        @Override
        public Iterator<JanusElement> execute(final GraphCentricQuery query, final JointIndexQuery indexQuery, final Object exeInfo, final QueryProfiler profiler) {
            Iterator<JanusElement> iter;
            if (!indexQuery.isEmpty()) {
                    ...
                    retrievals.add(new QueryUtil.IndexCall<Object>() {
                        ...
                        public Collection<Object> call(int limit) {
                            ...
                            try {
                                ...
                                throw new JanusException("Could not call index", e.getCause());
                            }
                        }
                    });
                }
                ...
                if (config.hasForceIndexUsage()) throw new JanusException("Could not find a suitable index to answer graph query and graph scans are disabled: " + query);
    ...
    public Function<Object, ? extends JanusElement> getConversionFunction(final ElementCategory elementCategory) {
    ...
    private final Function<Object, JanusVertex> vertexIDConversionFct = new Function<Object, JanusVertex>() {
        @Override
        public JanusVertex apply(@Nullable Object id) {
    ...
    private final Function<Object, JanusEdge> edgeIDConversionFct = new Function<Object, JanusEdge>() {
        @Override
        public JanusEdge apply(@Nullable Object id) {
            Preconditions.checkNotNull(id);
            Preconditions.checkArgument(id instanceof RelationIdentifier);
            return ((RelationIdentifier)id).findEdge(StandardJanusTx.this);
        }
    };

    private final Function<Object, JanusVertexProperty> propertyIDConversionFct = new Function<Object, JanusVertexProperty>() {
        @Override
        public JanusVertexProperty apply(@Nullable Object id) {
            Preconditions.checkNotNull(id);
            Preconditions.checkArgument(id instanceof RelationIdentifier);
            return ((RelationIdentifier)id).findProperty(StandardJanusTx.this);
        }
    };
    ...
    public synchronized void commit() {
        ...
        try {
            ...
            try {
                ...
                throw new JanusException("Could not rollback after a failed commit", e);
            }
            throw new JanusException("Could not commit transaction due to exception during persistence", e);
        } finally {
            releaseTransaction();
    ...
    public synchronized void rollback() {
        ...
        try {
            ...
            throw new JanusException("Could not rollback transaction due to exception", e);
        } finally {
            releaseTransaction();
            if (null != config.getGroupName() && !success) {
