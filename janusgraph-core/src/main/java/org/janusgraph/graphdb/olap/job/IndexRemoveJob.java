...
public class IndexRemoveJob extends IndexUpdateJob implements ScanJob {
    ...
    protected void validateIndexStatus() {
        if (index instanceof RelationTypeIndex) {
            ...
            if (gindex.isMixedIndex())
                throw new UnsupportedOperationException("Cannot remove mixed indexes through Janus. This can " +
    ...
    public List<SliceQuery> getQueries() {
        if (isGlobalGraphIndex()) {
            ...
            StandardJanusTx tx = (StandardJanusTx)graph.get().buildTransaction().readOnly().start();
            try {
