...
public class IndexRepairJob extends IndexUpdateJob implements VertexScanJob {
    ...
    protected void validateIndexStatus() {
        JanusSchemaVertex schemaVertex = mgmt.getSchemaVertex(index);
    ...
    public void process(JanusVertex vertex, ScanMetrics metrics) {
        try {
            ...
            if (index instanceof RelationTypeIndex) {
                ...
                for (JanusRelation relation : vertex.query().types(indexRelationTypeName).direction(Direction.OUT).relations()) {
                    InternalRelation janusRelation = (InternalRelation)relation;
                    for (int pos = 0; pos < janusRelation.getArity(); pos++) {
                        if (!wrappedType.isUnidirected(Direction.BOTH) && !wrappedType.isUnidirected(EdgeDirection.fromPosition(pos)))
                            continue; //Directionality is not covered
                        Entry entry = edgeSerializer.writeRelation(janusRelation, wrappedType, pos, writeTx);
                        additions.add(entry);
                    }
                }
                ...
                switch (indexType.getElement()) {
                    ...
                    case PROPERTY:
                        ...
                        for (JanusVertexProperty p : addIndexSchemaConstraint(vertex.query(),indexType).properties()) {
                            elements.add(p);
                        }
                        break;
                    case EDGE:
                        elements = Lists.newArrayList();
                        for (JanusEdge e : addIndexSchemaConstraint(vertex.query().direction(Direction.OUT),indexType).edges()) {
                            elements.add(e);
                        }
                        break;
                ...
                if (indexType.isCompositeIndex()) {
                    for (JanusElement element : elements) {
            ...
            throw new JanusException(e.getMessage(), e);
        }
    }
    ...
    private static<Q extends BaseVertexQuery> Q addIndexSchemaConstraint(Q query, IndexType indexType) {
        if (indexType.hasSchemaTypeConstraint()) {
            JanusSchemaType constraint = indexType.getSchemaTypeConstraint();
