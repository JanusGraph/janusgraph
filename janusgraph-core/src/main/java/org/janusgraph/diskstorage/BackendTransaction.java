...
public class BackendTransaction implements LoggableTransaction {
        ...
        if (storeFeatures.hasMultiQuery()) {
            ...
            if (threadPool == null || keys.size() < MIN_TASKS_TO_PARALLELIZE) {
                ...
                try {
                    ...
                    throw new JanusException("Interrupted while waiting for multi-query to complete", e);
                }
                if (failureCount.get() > 0) {
                    throw new JanusException("Could not successfully complete multi-query. " + failureCount.get() + " individual queries failed.");
                }
                for (int i=0;i<keys.size();i++) {
    ...
    private final <V> V executeRead(Callable<V> exe) throws JanusException {
