                        ...
                        if (opt.isManaged()) {
                            ...
                            if (opt.getType().equals(ConfigOption.Type.FIXED)) {
                                mut += "This setting is " + opt.getType() +
                                        " and cannot be changed after bootstrapping Janus.";
                            } else {
                                final String warning =
                                    "Settings with mutability " + opt.getType() + " are centrally managed in " +
                                    "Janus's storage backend.  After starting the database for the first time, " +
                                    "this file's copy of this setting is ignored.  Use Janus's Management " +
                                    "System to read or modify this value after bootstrapping.";
                                mut += WordUtils.wrap(warning, WRAP_COLUMNS, "\n# ", false);
                            }
                        }
                        String umbrella = null;
                        // This background info on umbrellas isn't critical when getting started with an Janus config
...
//                        }
        ...
        // Read what we just wrote.  Make sure it validates as a Janus config.
