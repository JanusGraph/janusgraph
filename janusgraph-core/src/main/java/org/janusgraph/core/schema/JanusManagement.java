...
/**
 * The JanusManagement interface provides methods to define, update, and inspect the schema of a Janus graph.
 * It wraps a {@link JanusTransaction} and therefore copies many of its methods as they relate to schema inspection
 * and definition.
 * <p/>
 * JanusManagement behaves like a transaction in that it opens a transactional scope for reading the schema and making
 * changes to it. As such, it needs to be explicitly closed via its {@link #commit()} or {@link #rollback()} methods.
 * A JanusManagement transaction is opened on a graph via {@link org.janusgraph.core.JanusGraph#openManagement()}.
 * <p/>
 * JanusManagement provides methods to:
 * <ul>
 ...
 */
public interface JanusManagement extends JanusConfiguration, SchemaManager {
    ...
    /**
     * Returns an {@link IndexBuilder} to add a graph index to this Janus graph. The index to-be-created
    ...
    public interface IndexBuilder {
         ...
         * @return this IndexBuilder
         */
        public IndexBuilder indexOnly(JanusSchemaType schemaType);
    ...
    /**
     * Retrieves the consistency modifier for the given {@link JanusSchemaElement}. If none has been explicitly
    ...
    /**
     * Sets the consistency modifier for the given {@link JanusSchemaElement}. Note, that only {@link RelationType}s
    ...
    /**
     * Retrieves the time-to-live for the given {@link JanusSchemaType} as a {@link Duration}.
     * If no TTL has been defined, the returned Duration will be zero-length ("lives forever").
     *
     * @param type
     * @return
     */
    public Duration getTTL(JanusSchemaType type);

    /**
     * Sets the time-to-live for the given {@link JanusSchemaType}. The most granular time unit used for TTL values
     ...
     */
    public void setTTL(JanusSchemaType type, Duration duration);
    ...
    /**
     * Changes the name of a {@link JanusSchemaElement} to the provided new name.
    ...
    /**
     * If an index update job was triggered through {@link #updateIndex(JanusIndex, SchemaAction)} with schema actions
    ...
    /**
     * Returns a set of unique instance ids for all Janus instances that are currently
    ...
    /**
     * Forcefully removes a Janus instance from this graph cluster as identified by its name.
     * <p/>
     * This method should be used with great care and only in cases where a Janus instance
     ...
     *
     * @see org.janusgraph.core.JanusTransaction#commit()
     */
    public void commit();
     ...
     *
     * @see org.janusgraph.core.JanusTransaction#rollback()
     */
