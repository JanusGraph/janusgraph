...
/**
 * This class supports iteration over Janus's ConfigNamespaces at runtime.
 *
 * Janus's ConfigOptions and ConfigNamespaces are defined by public static final fields
 * spread across more than ten classes in various janus modules/jars.  A ConfigOption
 ...
 * internally called by Janus to preload ConfigOptions when performing lookups or
 ...
 * Normally, only Janus internals should use this class.
 */
public enum ReflectiveConfigOptionLoader {
    INSTANCE;

    private static final String SYS_PROP_NAME = "janus.load.cfg.opts";
    ...
    public void loadStandard(Class<?> caller) {
        ...
        /*
         * Aside from the classes in janus-core, we can't guarantee the presence
         * of these classes at runtime.  That's why they're loaded reflectively.
         * We could probably hard-code the initialization of the janus-core classes,
