...
/**
 * Wraps a ColumnFamilyInputFormat and converts CFIF's binary types to Janus's binary types.
 */
public class CassandraBinaryInputFormat extends AbstractBinaryInputFormat {
    ...
    public void setConf(final Configuration config) {
        ...
        // Copy some Janus configuration keys to the Hadoop Configuration keys used by Cassandra's ColumnFamilyInputFormat
        ConfigHelper.setInputInitialAddress(config, janusConf.get(GraphDatabaseConfiguration.STORAGE_HOSTS)[0]);
        if (janusConf.has(GraphDatabaseConfiguration.STORAGE_PORT))
            ConfigHelper.setInputRpcPort(config, String.valueOf(janusConf.get(GraphDatabaseConfiguration.STORAGE_PORT)));
        if (janusConf.has(GraphDatabaseConfiguration.AUTH_USERNAME))
            ConfigHelper.setInputKeyspaceUserName(config, janusConf.get(GraphDatabaseConfiguration.AUTH_USERNAME));
        if (janusConf.has(GraphDatabaseConfiguration.AUTH_PASSWORD))
            ConfigHelper.setInputKeyspacePassword(config, janusConf.get(GraphDatabaseConfiguration.AUTH_PASSWORD));
        ...
        ConfigHelper.setInputColumnFamily(config, janusConf.get(AbstractCassandraStoreManager.CASSANDRA_KEYSPACE),
                mrConf.get(JanusHadoopConfiguration.COLUMN_FAMILY_NAME), wideRows);
        log.debug("Set keyspace: {}", janusConf.get(AbstractCassandraStoreManager.CASSANDRA_KEYSPACE));
        ...
        predicate.setSlice_range(getSliceRange(JanusHadoopSetupCommon.DEFAULT_SLICE_QUERY, rangeBatchSize)); // TODO stop slicing the whole row
