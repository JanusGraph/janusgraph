...
public class CassandraScanJobIT extends JanusGraphBaseTest {
    ...
    public void testPartitionedVertexScan() throws Exception {
        ...
        finishSchema();
        JanusVertex supernode = graph.addVertex("part");
        for (int i = 0; i < 128; i++) {
            JanusVertex v = graph.addVertex("part");
        ...
        c.set(ConfigElement.getPath(JanusHadoopConfiguration.GRAPH_CONFIG_KEYS, true) + "." + "storage.cassandra.keyspace", getClass().getSimpleName());
        c.set(ConfigElement.getPath(JanusHadoopConfiguration.GRAPH_CONFIG_KEYS, true) + "." + "storage.backend", "cassandrathrift");
    ...
    public void testPartitionedVertexFilteredScan() throws Exception {
        ...
        c.set(ConfigElement.getPath(JanusHadoopConfiguration.GRAPH_CONFIG_KEYS, true) + "." + "storage.cassandra.keyspace", getClass().getSimpleName());
        c.set(ConfigElement.getPath(JanusHadoopConfiguration.GRAPH_CONFIG_KEYS, true) + "." + "storage.backend", "cassandrathrift");
        c.set(ConfigElement.getPath(JanusHadoopConfiguration.FILTER_PARTITIONED_VERTICES), "true");
        c.set("cassandra.input.partitioner.class", "org.apache.cassandra.dht.Murmur3Partitioner");
