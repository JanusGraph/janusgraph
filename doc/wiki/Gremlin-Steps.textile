[[https://github.com/tinkerpop/gremlin/raw/master/doc/images/gremlin-standing-small.png|align=left|float]] [[https://github.com/thinkaurelius/faunus/raw/master/doc/images/faunus-head.png|width=105px|align=right|float]] "Gremlin":http://gremlin.tinkerpop.com is a graph traversal language developed by "TinkerPop":http://tinkerpop.com. Faunus' Gremlin steps are implemented in "MapReduce":http://hadoop.apache.org/mapreduce/. For sake of clarity, the Gremlin that compiles down to "Pipes":http://pipes.tinkerpop.com and is used for real-time traversals against a "graph database":http://en.wikipedia.org/wiki/Graph_database is denoted Gremlin/Pipes. The Gremlin that is distributed with Faunus and compiles down to MapReduce is denoted Gremlin/Faunus.

<br/>

There are major conceptual differences between Gremlin/Pipes and Gremlin/Faunus. These differences are itemized below.

  * Gremlin/Faunus is "breadth-first":http://en.wikipedia.org/wiki/Breadth-first_search. Gremlin/Pipes is "depth-first":http://en.wikipedia.org/wiki/Depth-first_search.
...
|_. Gremlin/Faunus step |_. Description |_. Reduce? |
| @step(closure,closure,class...)@ | a generic Groovy MapReduce with provided closures | true |
|_. transforms | | |
...
| @linkOut(step,label,key?)@ | create outgoing edge to previous vertices with @key?@ being a weight property | true |
| @linkIn(step,label,key?)@ | create incoming edge to previous vertices with @key?@ being a weight property | true |
...
h2. Note on Path-Based Traversals

When a computation requires the history of a traversal, "combinatoric":http://en.wikipedia.org/wiki/Combinatorics explosions can occur which are expensive in terms of both space (the size of the output) and time (computing the output). The following steps are steps that turn *on* path computations. By default, path computations are turned *off* unless @enablePath()@ is explicitly called within the expression. 
