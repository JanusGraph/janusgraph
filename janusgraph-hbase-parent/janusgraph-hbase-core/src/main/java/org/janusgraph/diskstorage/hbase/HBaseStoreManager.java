...
public class HBaseStoreManager extends DistributedStoreManager implements KeyColumnValueStoreManager {
    ...
    public static final ConfigOption<Boolean> SHORT_CF_NAMES =
            ...
            "Whether to shorten the names of Janus's column families to one-character mnemonics " +
    ...
    public static final ConfigOption<String> COMPRESSION =
            ...
            "The compression algorithm must be installed and available on the HBase cluster.  Janus cannot install " +
    ...
    public static final ConfigOption<Boolean> SKIP_SCHEMA_CHECK =
            ...
            "Assume that Janus's HBase table and column families already exist. " +
            "When this is true, Janus will not check for the existence of its table/CFs, " +
            "nor will it attempt to create them under any circumstances.  This is useful " +
            "when running Janus without HBase admin privileges.",
            ConfigOption.Type.MASKABLE, false);

    public static final ConfigOption<String> HBASE_TABLE =
            new ConfigOption<String>(HBASE_NS, "table",
            "The name of the table Janus will use.  When " + ConfigElement.getPath(SKIP_SCHEMA_CHECK) +
            " is false, Janus will automatically create this table if it does not already exist.",
            ConfigOption.Type.LOCAL, "janus");
    ...
    /**
     * The total number of HBase regions to create with Janus's table. This
     * setting only effects table creation; this normally happens just once when
     * Janus connects to an HBase backend for the first time.
     */
    public static final ConfigOption<Integer> REGION_COUNT =
            new ConfigOption<Integer>(HBASE_NS, "region-count",
            "The number of initial regions set when creating Janus's HBase table",
     ...
     * <p/>
     * If Janus's HBase table does not exist, then it will be created with total
    ...
    public static final ConfigOption<Integer> REGIONS_PER_SERVER =
            ...
            "The number of regions per regionserver to set when creating Janus's HBase table",
     ...
     * <p>
     * When this <b>is not</b> set, Janus attempts to automatically detect the
     * HBase runtime version by calling {@link VersionInfo#getVersion()}. Janus
     ...
     * if a match is found.
     * <p>
     * When this <b>is</b> set, Janus will not call
     * {@code VersionInfo.getVersion()} or read its hard-coded list of supported
     * version prefixes. Janus will instead attempt to instantiate the class
     * specified (via the no-arg constructor which must exist) and then attempt
     * to cast it to HBaseCompat and use it as such. Janus will assume the
     ...
     * useful to users who want to run against a new release of HBase that Janus
    ...
    public static final ConfigOption<String> COMPAT_CLASS =
            ...
            "When this option is unset, Janus calls HBase's VersionInfo.getVersion() and loads the matching compat class " +
            "at runtime.  Setting this option forces Janus to instead reflectively load and instantiate the specified class.",
    ...
    public HBaseStoreManager(org.janusgraph.diskstorage.configuration.Configuration config) throws BackendException {
        ...
        if (config.has(REGIONS_PER_SERVER) && config.has(REGION_COUNT)) {
            logger.warn("Both {} and {} are set in Janus's configuration, but "
...
//        }
     ...
     * region's start and end key bounds using Janus-partitioning-friendly
     ...
     * @return Janus-friendly expression of each region's rowkey boundaries
     */
    private Map<KeyRange, ServerName> normalizeKeyBounds(NavigableMap<HRegionInfo, ServerName> raw) {
        ...
        for (Map.Entry<HRegionInfo, ServerName> e : raw.entrySet()) {
            ...
            if (null == startKey && null == endKey) {
                ...
                // Convert HBase's inclusive end keys into exclusive Janus end keys
    ...
    /**
     * Convert Janus internal Mutation representation into HBase native commands.
     *
     * @param mutations    Mutations to convert into HBase commands.
     * @param putTimestamp The timestamp to use for Put commands.
     * @param delTimestamp The timestamp to use for Delete commands.
     * @return Commands sorted by key converted from Janus internal representation.
    ...
    private void checkConfigDeprecation(org.janusgraph.diskstorage.configuration.Configuration config) {
        if (config.has(GraphDatabaseConfiguration.STORAGE_PORT)) {
            logger.warn("The configuration property {} is ignored for HBase. Set hbase.zookeeper.property.clientPort in hbase-site.xml or {}.hbase.zookeeper.property.clientPort in Janus's configuration file.",
