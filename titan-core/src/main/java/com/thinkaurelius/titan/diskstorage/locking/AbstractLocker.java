...
public abstract class AbstractLocker<S extends LockStatus> implements Locker {
    ...
    public static abstract class Builder<S, B extends Builder<S, B>> {
        ...
        public B logger(Logger log) {
            this.log = log; return self();
        }
        
        public B lockExpireNS(long exp, TimeUnit unit) {
            this.lockExpireNS = NANOSECONDS.convert(exp, unit); return self();
        }
        ...
        public B internalState(LockerState<S> state) {
            this.lockState = state; return self();
        }
     ...
     *            identifies the lock
     * @param tx
     *            identifies the process claiming this lock
     ...
     * 
     * @param lockID
     *            identifies the lock to check
     * @param lockStatus
     *            the result of a prior successful {@code writeSingleLock(...)}
     ...
     * @param tx
     *            identifies the process that wrote and checked this lock
     * @throws Throwable
    ...
    private boolean lockLocally(KeyColumn lockID, StoreTransaction tx) {
        return lockLocally(lockID, times.getApproxNSSinceEpoch(false) + lockExpireNS, tx);
    }
