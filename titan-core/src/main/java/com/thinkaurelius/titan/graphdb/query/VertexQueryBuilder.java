...
public class VertexQueryBuilder implements AtomicQuery {
    ...
    public VertexQueryBuilder(InternalTitanTransaction tx) {
    ...
    public VertexQueryBuilder(InternalVertex n) {
    ...
    public VertexQueryBuilder(InternalTitanTransaction tx, long nodeid) {
        this(tx);
    ...
    VertexQueryBuilder(VertexQueryBuilder q) {
        dir = q.dir;
        type = q.type;
        group = q.group;
    ...
    VertexQueryBuilder(InternalVertex node, VertexQueryBuilder q) {
        this(q);
        this.node = node;
    }

    @Override
    public VertexQueryBuilder clone() {
        VertexQueryBuilder q = new VertexQueryBuilder(this);
        return q;
    }

    public static final VertexQueryBuilder queryAll(InternalVertex node) {
        return new VertexQueryBuilder(node).includeHidden();
    }
    ...
    @Override
    public <T extends Comparable<T>> AtomicQuery has(String key, T value, Compare compare) {
        TitanKey k = getKey(key);
        if (k == null) return EmptyAtomicQuery.INSTANCE;
        else return has(k, value, compare);
    }

    public <T extends Comparable<T>> AtomicQuery has(TitanKey key, T value, Compare compare) {
...
//          Preconditions.checkNotNull(interval,"Additional constraint leads to empty intersection and is therefore infeasible!");
    ...
    @Override
    public VertexQueryBuilder type(TitanType type) {
        Preconditions.checkNotNull(type);
        tx.verifyAccess(type);
    ...
    @Override
    public VertexQueryBuilder group(TypeGroup group) {
    ...
    @Override
    public VertexQueryBuilder direction(Direction d) {
        dir = d;
        return this;
    }

    @Override
    public VertexQueryBuilder onlyModifiable() {
        queryUnmodifiable = false;
        return this;
    }

    @Override
    public VertexQueryBuilder includeHidden() {
        queryHidden = true;
        return this;
    }

    @Override
    public VertexQueryBuilder limit(long limit) {
        this.limit = limit;
        return this;
    }

    @Override
    public VertexQueryBuilder inMemory() {
    ...
    private VertexListInternal retrieveFromMemory(VertexListInternal vertices) {
        ...
        VertexQueryBuilder q = new VertexQueryBuilder(node, this);
