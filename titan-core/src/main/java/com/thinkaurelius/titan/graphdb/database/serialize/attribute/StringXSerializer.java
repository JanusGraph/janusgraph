...
public class StringXSerializer implements AttributeSerializer<StringX>, SupportsNullSerializer {
    ...
    @Override
    public void verifyAttribute(StringX value) {
        Preconditions.checkArgument(value.length()<=MAX_LENGTH,"String is too long: %s",value.length());
    }

    @Override
    public StringX convert(Object value) {
        Preconditions.checkNotNull(value);
        return new StringX(value.toString());
    }


    @Override
    public StringX read(ScanBuffer buffer) {
    ...
    @Override
    public void writeObjectData(WriteBuffer buffer, StringX attribute) {
        ...
        if (attribute==null) {
            ...
            if (attribute.length()>TEXT_COMRPESSION_THRESHOLD && StringEncoding.isAsciiString(attribute.getString())) {
                compression=CompressionType.SMAZ;
            } else {
        ...
        if (compression==CompressionType.NO_COMPRESSION) {
            ...
            if (StringEncoding.isAsciiString(value)) {
                if (value.length()==0) VariableLong.writePositive(buffer,1l<<NO_COMPRESSION_OFFSET);
                ...
                VariableLong.writePositive(buffer,(((long)value.length())<<NO_COMPRESSION_OFFSET) + (1l<<COMPRESSOR_BIT_LEN)); //Marker for full UTF encoding
                for (int i = 0; i < value.length(); i++) { //variable encoding of the characters
                    int c = value.charAt(i);
                    if (c <= 0x007F) {
            ...
            byte[] compressed = compression.compress(value);
    ...
    private enum CompressionType {
        ...
        SMAZ {
            ...
            private final Smaz smaz = new Smaz();
            ...
            @Override
            public String decompress(ScanBuffer buffer, int numBytes) {
                return smaz.decompress(buffer,numBytes);
            }
        },
